$(function() {
    $("body").on("click", "ul.acc-menu a", function() {
        var d = $(this).closest("ul.acc-menu").children("li");
        $(this).closest("li").addClass("clicked");
        $.each(d, function(e) {
            if ($(d[e]).hasClass("clicked")) {
                $(d[e]).removeClass("clicked");
                return true
            }
            if ($.cookie("admin_leftbar_collapse") !== "collapse-leftbar" || $(this).parents(".acc-menu").length > 1) {
                $(d[e]).find("ul.acc-menu:visible").slideToggle()
            }
            $(d[e]).removeClass("open")
        });
        ...
        if ($.cookie("admin_leftbar_collapse") !== "collapse-leftbar" || $(this).parents(".acc-menu").length > 1) {
            $(this).siblings("ul.acc-menu").slideToggle({duration: 200, progress: function() {
                    checkpageheight();
    ...
    while (true) {
        c.addClass("active");
        c.closest("ul.acc-menu").show().closest("li").addClass("open");
        c = $(c).parents("li").eq(0);
        if ($(c).parents("ul.acc-menu").length <= 0) {
            break
        }
    }
    var a = $("li").filter(function() {
    ...
    $("#woo").click(function() {
        $.ajax({type: "POST", dataType: "html", url: "model/mark-view.php", success: function(d) {
                $("#view-message").removeClass("badge")
            }, error: function(d) {
                console.log("valor: " + d)
            }})
    })
...
function leftbarTopPos() {
    var a = $("body.static-header").scrollTop();
    if (a < 41) {
        $("ul#sidebar").css("top", 40 - a + "px")
    } else {
...
$("#headerbardropdown").click(function(a) {
...
function cargarCombo(a, e, b, d, g, c, f) {
    $.post("catalogos/obtenerCatalogo.php", {table: e, index: b, description: d, query: g, nodo: f}, function(h, j, i) {
        $(h.list).each(function() {
            var k = $("<option>").val(this.index).text(this.description).data("idNodo", this.nodo);
            $(k).attr("nodo", this.nodo);
            if (c != "") {
                if (c == this.index) {
